...
# MODIFIED: Added raw_data_zero_dropout_rate parameter
...
# --- CNN-LSTM Model with Corrected Temporal Input (No Temporal Dropout) ---
class CNNLSTMWithAttention(nn.Module):
    def __init__(self, img_rows, img_cols, num_classes):
        ...
        # Calculate input size for LSTM based on corrected reshape
        cnn_out_channels = 32
        freq_bins_after_pooling = img_rows // 2 # 256 // 2 = 128
        
        self.lstm_input_size = cnn_out_channels * freq_bins_after_pooling # 32 * 128 = 4096
        
        self.lstm = nn.LSTM(input_size=self.lstm_input_size, hidden_size=64, batch_first=True) 
        self.dropout2 = nn.Dropout(0.5) # Standard dropout on LSTM output
    ...
    def forward(self, x):
        ...
        # --- Correct Reshape for sequential LSTM processing ---
        # Current x shape: (batch_size, channels, freq_bins, time_bins) e.g., (B, 32, 128, 11)
        # Desired x_lstm_input shape: (batch_size, time_bins, channels * freq_bins) e.g., (B, 11, 4096)
        ...
        # Take the output from the LAST time step of the LSTM for classification
        x = lstm_out[:, -1, :] # Shape: (batch, hidden_size) e.g., (B, 64)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, epochs, device, 
    ...
    # We will load checkpoint if it exists to resume training for this specific dropout rate
    start_epoch = 0
...
# No need for a separate load_checkpoint function anymore, it's integrated into train_model

# --- Main Execution Block ---
if __name__ == "__main__":
    print("Starting PyTorch CNN-LSTM Model Training with Raw Data Zero-Dropout Experiment...")
    
    # --- CONFIGURE THE DESIRED DROPOUT RATE HERE FOR THIS RUN ---
    # Set this value to 0.0, 0.3, 0.5, or 0.7 for your individual runs.
    RAW_DATA_ZERO_DROPOUT_RATE = 0.5 # Example: Setting to 0.0 for a baseline run
    # -----------------------------------------------------------
    ...
    model = CNNLSTMWithAttention(IMG_ROWS, IMG_COLS, NUM_CLASSES).to(DEVICE)
    ...
    checkpoint_filename_for_this_run = f'best_cnn_lstm_raw_0.3_dropout_{int(RAW_DATA_ZERO_DROPOUT_RATE*100)}.pth'
    ...
    print("\nPyTorch CNN-LSTM (Raw Data Zero-Dropout Experiment) process finished.")
