...
# The filenames will be dynamically set based on the dropout rate for this specific run
...
class CNN_NCDE_Model(nn.Module):
    def __init__(self, num_classes=2, cnn_dropout_rate=0.25, ncde_hidden_dim=64, ncde_dropout_rate=0.5):
        ...
        cnn_out_channels = 32
        freq_bins_after_pooling = IMG_ROWS // 2 
        self.ncde_input_dim = cnn_out_channels * freq_bins_after_pooling 
...
# --- Training and Evaluation Functions (MODIFIED FOR LATEST CHECKPOINTING) ---
def train_model(model, train_loader, val_loader, criterion, optimizer, epochs, device, 
                checkpoint_dir, best_checkpoint_filename, latest_checkpoint_filename):
    """
    Trains the PyTorch model with two types of checkpointing:
    - Saves the model with the best validation accuracy.
    - Saves the model from the last completed epoch (for exact resumption).
    """
    model.train() 
    best_val_accuracy = -1.0 
    start_epoch = 0

    full_latest_checkpoint_path = os.path.join(checkpoint_dir, latest_checkpoint_filename)
    full_best_checkpoint_path = os.path.join(checkpoint_dir, best_checkpoint_filename)

    # --- LOADING LOGIC: Prioritize latest, then best, then start fresh ---
    if os.path.exists(full_latest_checkpoint_path):
        print(f"Loading LATEST checkpoint from {full_latest_checkpoint_path}...")
        checkpoint = torch.load(full_latest_checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch'] + 1 
        # When resuming from latest, also get the best_val_accuracy seen so far
        # in case the latest wasn't the best overall
        if os.path.exists(full_best_checkpoint_path):
            best_chkpt = torch.load(full_best_checkpoint_path, map_location=device)
            best_val_accuracy = best_chkpt.get('val_accuracy', -1.0)
        print(f"Resuming from Epoch {start_epoch} (from latest checkpoint). Best Val Acc so far: {best_val_accuracy:.4f}")
    elif os.path.exists(full_best_checkpoint_path):
        print(f"No LATEST checkpoint found. Loading BEST checkpoint from {full_best_checkpoint_path}...")
        checkpoint = torch.load(full_best_checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch'] + 1 
        best_val_accuracy = checkpoint.get('val_accuracy', -1.0) 
        print(f"Resuming from Epoch {start_epoch} (from best checkpoint). Best Val Acc so far: {best_val_accuracy:.4f}")
    else:
        print("No checkpoints found. Starting training from scratch (Epoch 0).")
    ...
    for epoch in range(start_epoch, epochs): 
        ...
        torch.save({
            ...
            'loss': epoch_loss,
            'val_accuracy': val_accuracy, # Store current val_accuracy for potential future resuming as best
        }, full_latest_checkpoint_path)
        print(f"   LATEST Checkpoint saved: {full_latest_checkpoint_path}")

        # --- Save the BEST checkpoint (if performance improved) ---
        if val_accuracy > best_val_accuracy:
            ...
            }, full_best_checkpoint_path)
            print(f"   BEST Checkpoint saved: {full_best_checkpoint_path} (Val Acc: {val_accuracy:.4f})")
...
if __name__ == "__main__":
    ...
    RAW_DATA_ZERO_DROPOUT_RATE = 0.5 # Example: Setting to 0.3 for a dropout run
    # -----------------------------------------------------------
    ...
    # Dynamic checkpoint filenames to distinguish runs
    # Example: best_cnn_ncde_raw_data_dropout_30_best.pth and best_cnn_ncde_raw_data_dropout_30_latest.pth
    base_checkpoint_name = f'cnn_ncde_raw_data_dropout_{int(RAW_DATA_ZERO_DROPOUT_RATE*100)}'
    best_checkpoint_filename_for_this_run = f'{base_checkpoint_name}_best.pth'
    latest_checkpoint_filename_for_this_run = f'{base_checkpoint_name}_latest.pth'
    ...
    train_model(model, train_loader, val_loader, criterion, optimizer, 
                        ...
                        best_checkpoint_filename=best_checkpoint_filename_for_this_run,
                        latest_checkpoint_filename=latest_checkpoint_filename_for_this_run)
    print("Model training complete.")

    # 4. Evaluate the Model on Test Data for the current dropout rate
    # For final evaluation, always load the BEST performing model
    final_model_path = os.path.join(CHECKPOINT_DIR, best_checkpoint_filename_for_this_run)
    if os.path.exists(final_model_path):
        checkpoint = torch.load(final_model_path, map_location=DEVICE)
        model.load_state_dict(checkpoint['model_state_dict'])
        print(f"Loaded BEST model for final evaluation from {final_model_path}")
    else:
        # Fallback in case best was never saved (e.g., very short training run, or high dropout makes all models bad)
        print(f"WARNING: Best model not found at {final_model_path}. Loading LATEST model for evaluation if available.")
        if os.path.exists(full_latest_checkpoint_path):
            checkpoint = torch.load(full_latest_checkpoint_path, map_location=DEVICE)
            model.load_state_dict(checkpoint['model_state_dict'])
            print(f"Loaded LATEST model for final evaluation from {full_latest_checkpoint_path}")
        else:
            print("WARNING: Neither best nor latest model found. Evaluating current model state (might not be trained).")
